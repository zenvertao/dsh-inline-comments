window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = ".ic-root{position:fixed;inset:0;pointer-events:none;z-index:2147483000;}"
			...
			+ ".ic-pill .dot{width:16px;height:16px;border-radius:50%;box-sizing:border-box;background:var(--dsw-alias-state-business-primary,#3b6fff);color:#fff;font:700 9px/16px sans-serif;display:grid;place-items:center;flex:none;}"
			+ ".ic-pill .x{position:absolute;right:5px;top:50%;transform:translateY(-50%);width:16px;height:16px;border-radius:50%;background:#d1d5db;color:#333;font:700 12px/16px -apple-system,system-ui,sans-serif;display:grid;place-items:center;cursor:pointer;opacity:0;transition:opacity .15s;}"
		...
		function apply(ctx) {
			...
			function positionEditor(){
				...
				if (!rect) return;
				var x = rect.right + 14;
				if (x + editor.offsetWidth > viewport().w) x = rect.left - editor.offsetWidth - 14;
				if (x < 2) x = 2;
				var y = rect.top;
				setPos(editor, x, y);
				syncFloatVis(editor);
			...
			// ==== Interaction wiring ====
			document.addEventListener("scroll", function(){ if (window.requestAnimationFrame) requestAnimationFrame(repositionFloats); else repositionFloats(); }, { capture: true, passive: true });
