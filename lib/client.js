window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function vvOffset(){ if (typeof window === "undefined" || !window.visualViewport) return { x: 0, y: 0 }; var ua = (typeof navigator !== "undefined" && navigator.userAgent) ? navigator.userAgent : ""; var safari = ua.indexOf("Safari") >= 0 && ua.indexOf("Chrome") < 0 && ua.indexOf("Chromium") < 0 && ua.indexOf("CriOS") < 0 && ua.indexOf("Edg") < 0 && ua.indexOf("OPR") < 0 && ua.indexOf("Firefox") < 0; if (!safari) return { x: 0, y: 0 }; return { x: window.visualViewport.offsetLeft || 0, y: window.visualViewport.offsetTop || 0 }; }
			...
			function setPos(el, x, y){ var off = vvOffset(); el.style.left = (Math.max(2, Math.min(x, viewport().w - el.offsetWidth - 4)) + off.x) + "px"; el.style.top = (Math.max(2, y) + off.y) + "px"; }
			...
			function drawAll(){ syncSession(); redraw(); positionAffordance(); }
			...
			function updatePill(){
				...
				if (box){
					...
					pill.style.left = (Math.max(2, Math.min(px, viewport().w - aw - 4)) + vvOffset().x) + "px";
					pill.style.top = (Math.max(2, Math.min(py, viewport().h - ah - 4)) + vvOffset().y) + "px";
				} else {
