window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS = ".ic-root{position:fixed;inset:0;pointer-events:none;z-index:2147483000;}"
			...
			+ ".ic-badge{position:absolute;pointer-events:auto;z-index:6;min-width:22px;height:22px;padding:0;border-radius:11px;background:var(--dsw-alias-state-business-primary,#3b6fff);color:#fff;font:700 11px/1 -apple-system,system-ui,sans-serif;display:flex;align-items:center;justify-content:center;cursor:pointer;border:1.5px solid rgba(255,255,255,.95);box-sizing:border-box;box-shadow:0 2px 5px rgba(0,0,0,.28);}"
			...
			+ ".ic-pill{position:absolute;z-index:32;display:inline-flex;align-items:center;gap:7px;border:1px solid var(--dsw-alias-border-l2,#e2e5ec);border-radius:20px;padding:5px 17px 5px 11px;font:12px -apple-system,system-ui,sans-serif;color:var(--dsw-alias-label-primary,#111);background:var(--dsw-alias-bg-layer-3,#fff);cursor:pointer;box-shadow:var(--dsw-shadow-lv2,0 2px 8px rgba(0,0,0,.08));pointer-events:auto;}"
			...
			+ ".ic-pill .x{position:absolute;right:5px;top:50%;transform:translateY(-50%);width:16px;height:16px;border-radius:50%;background:#d1d5db;color:#333;display:grid;place-items:center;cursor:pointer;opacity:0;transition:opacity .15s;}"
		...
		function apply(ctx) {
			...
			var pillX = document.createElement("span"); pillX.className="x"; pillX.innerHTML='<svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="3.2" stroke-linecap="round" aria-hidden="true"><line x1="7" y1="7" x2="17" y2="17"/><line x1="17" y1="7" x2="7" y2="17"/></svg>';
			...
			function drawAnn(ann, seq){
				...
				// float the badge one line above the selection end; anchor to a container that
				// doesn't clip (overflow:hidden) so it stays visible above tool rows too.
				var badgeHost = floatAnchor(range.startContainer);
				var bbase = badgeHost.getBoundingClientRect();
				b.style.left = (last.right - bbase.left - 6) + "px";
				b.style.top = (last.top - bbase.top - 24) + "px";
				...
				badgeHost.appendChild(b); ann._badge = b;
			}
			...
			function clipsChildren(el){
				if (!el || el.nodeType !== 1 || el === document.body) return false;
				try {
					var cs = window.getComputedStyle(el);
					return cs.overflowX === "hidden" || cs.overflowX === "clip" || cs.overflowY === "hidden" || cs.overflowY === "clip";
				} catch(e){ return false; }
			}
			// The floating affordance sits one line above the selection; a container that clips
			// (overflow:hidden) — e.g. a tool-call row — would hide it. Anchor to the nearest
			// ancestor that doesn't clip its children instead.
			function floatAnchor(node){
				var target = annotContainer(node);
				var scroll = document.querySelector("[data-conversation-scroll]");
				// stop at the conversation scrollport so the badge/affordance never float
				// above the composer or header (whose z-index is higher than content)
				while (target && target !== document.body && target !== scroll && clipsChildren(target) && target.parentElement) target = target.parentElement;
				if (target && target !== document.body){ try { if (window.getComputedStyle(target).position === "static") target.style.position = "relative"; } catch(e){} }
				return target;
			}
			function positionAffordance(){
				...
				var target = floatAnchor(pending.start);
			...
			// send-button click: inject in the capture phase so it lands before the composer
			// reads the draft, without disturbing the button mid-click (mousedown was racy)
