window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function setPos(el, x, y){ el.style.left = Math.max(2, Math.min(x, viewport().w - el.offsetWidth - 4)) + "px"; el.style.top = Math.max(2, y) + "px"; }
			...
			function updatePill(){
				...
				if (box){
					...
					pill.style.left = Math.max(2, Math.min(px, viewport().w - aw - 4)) + "px";
					pill.style.top = Math.max(2, Math.min(py, viewport().h - ah - 4)) + "px";
				} else {
